...
controls_ui <- function(id,
                        controls = c("preprocess", "filters", "labs", "parameters", "appearance", "code"),
                        insert_code = FALSE) {
  ns <- NS(id)
  if (!is.null(controls)) {
    controls <- match.arg(
      controls,
      choices = c("preprocess", "labs", "parameters", "appearance", "filters", "code"),
      several.ok = TRUE
    )
  } else {
    return(tags$div(
  ...
  tagList(
    tags$div(
      ...
      if (isTRUE("preprocess" %in% controls)) {
        dropdown_(
          controls_preprocessing(ns),
          inputId = ns("controls-preprocess"),
          class = "esquisse-controls-filters",
          style = "default",
          label = mgi18n("preprocess"),
          up = TRUE,
...
controls_labs <- function(ns) {
  tags$div(
    ...
    labs_options_input(
      ...
      placeholder = mgi18n("Title"),
      label = mgi18n("Title:"),
      defaults = get_labs_defaults("title")
    ),
    labs_options_input(
      inputId = ns("labs_subtitle"),
      placeholder = mgi18n("Subtitle"),
      label = mgi18n("Subtitle:"),
      defaults = get_labs_defaults("subtitle")
    ),
    labs_options_input(
      inputId = ns("labs_caption"),
      placeholder = mgi18n("Caption"),
      label = mgi18n("Caption:"),
      defaults = get_labs_defaults("caption")
    ),
    labs_options_input(
      inputId = ns("labs_x"),
      placeholder = mgi18n("X label"),
      label = mgi18n("X label:"),
      defaults = get_labs_defaults("x")
    ),
    labs_options_input(
      inputId = ns("labs_y"),
      placeholder = mgi18n("Y label"),
      label = mgi18n("Y label:"),
      defaults = get_labs_defaults("y")
    ),
    tags$div(
      id = ns("controls-labs-fill"),
      style = "display: none;",
      textInput(inputId = ns("labs_fill"), placeholder = mgi18n("Fill label"), label = mgi18n("Fill label:"))
    ),
    tags$div(
      id = ns("controls-labs-color"),
      style = "display: none;",
      textInput(inputId = ns("labs_color"), placeholder = mgi18n("Color label"), label = mgi18n("Color label:"))
    ),
    tags$div(
      id = ns("controls-labs-size"),
      style = "display: none;",
      textInput(inputId = ns("labs_size"), placeholder = i18n("Size label"), label = mgi18n("Size label:"))
    ),
    tags$div(
      id = ns("controls-labs-shape"),
      style = "display: none;",
      textInput(inputId = ns("labs_shape"), placeholder = mgi18n("Shape label"), label = mgi18n("Shape label:"))
    )
  )
}
    ...
    tags$div(
      ...
      dropMenu(
        ...
        numericInput(
          ...
          label = mgi18n("Font size:"),
          value = defaults$size
        ),
        prettyRadioButtons(
          inputId = paste0(inputId, "_align"),
          label = mgi18n("Align:"),
          choiceNames = c(mgi18n("Left"), mgi18n("Center"), mgi18n("Right")),
...
#' Controls for preprocessing
controls_preprocessing <- function(ns){
  tagList(
    tags$label(
      class = "control-label",
      `for` = ns("vol_scatter"),
      mgi18n("log p-value/p-adjust:")
    ),
    prettyToggle(
      inputId = ns("vol_scatter"),
      label_on = mgi18n("Yes"),
      status_on = "success",
      status_off = "danger",
      label_off = mgi18n("No"),
...
controls_params <- function(ns) {
  ...
  tagList(
    ...
    tags$div(
      ...
      sliderInput(
        inputId = ns("size"),
        label = mgi18n("Size for points/lines:"),
        min = 0.5,
        max = 4,
        value = 1.2,
        width = "100%"
    ...
    tags$div(
      ...
      numericRangeInput(
        inputId = ns("xlim"),
        label = mgi18n("X-Axis limits (empty for none):"),
        value = c(NA, NA)
      ),
      selectInput(
        inputId = ns("transX"),
        label = mgi18n("X-Axis transform:"),
        selected = "identity",
    ...
    tags$div(
      ...
      numericRangeInput(
        inputId = ns("ylim"),
        label = mgi18n("Y-Axis limits (empty for none):"),
        value = c(NA, NA)
      ),
      selectInput(
        inputId = ns("transY"),
        label = mgi18n("Y-Axis transform:"),
        selected = "identity",
        choices = scales_trans,
        width = "100%"
      )
    ...
    tags$div(
      ...
      sliderInput(
        inputId = ns("adjust"),
        label = mgi18n("Bandwidth adjustment:"),
        min = 0.2,
        max = 6,
        value = 1,
        step = 0.1,
  ...
  tagList(
    ...
    if (insert_code) {
      actionLink(
        ...
        label = tagList(ph("arrow-circle-left"), mgi18n("Insert code in script"))
      )
    },
    tags$br()
  )
