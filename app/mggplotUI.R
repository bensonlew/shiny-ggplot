...
mggplotUI <- function(id, header = TRUE,
  ...
  box_title <- tags$div(
    ...
    tags$h1("chart_tools", class = "esquisse-title"),
    tags$div(
      class = "pull-right",
      actionButton(
  ...
  addin <- fillPage(tags$div(
    ...
    #   tags$div(
    #     style = "height: 100%; width: 50%; min-height: 400px;",
    #   )
    # ),
    ...
    controls_ui(
      ...
      controls = if (isTRUE(disable_filters)) {
        c("preprocess", "labs", "parameters", "appearance", "code")
      } else {
        c("preprocess", "filters", "labs", "parameters", "appearance", "code")
      }
    )
  ))
